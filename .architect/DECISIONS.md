...
- **Обоснование**:
  - TOTAL_1_5: 22 settled, win_pct 13.6%, ROI -53.6%. Poisson-based вероятности
    для экстремальных линий ненадёжны. Avg_odd 3.43 = модель ставит UNDER 1.5
    по высоким коэффициентам и систематически проигрывает.
  - TOTAL_3_5: 23 settled, ROI -32.0%. Та же проблема — крайние линии.
  - BTTS: 43 settled, ROI -16.4%. BTTS вероятности из joint Poisson не проходят
    через стэкинг (стэкинг обучен на 1X2 features). Без собственной калибровки
    не работает.
  - Odds > 2.50: ROI -19.2% (bucket 2.50-3.00) и -18.6% (bucket 3.00+).
    Системная просадка на longshots.
- **Оставлены**:
  - 1X2: 61 settled, ROI +9.9% — единственный стабильно прибыльный рынок
  - TOTAL 2.5: 146 settled, ROI -4.1% — в пределах variance, крупная выборка
  - DC: 9 settled — недостаточно данных, оставлен для наблюдения
- **Условия реактивации**:
  - TOTAL_1_5/3_5: после создания отдельной модели для крайних линий
    (COM-Poisson или xG-based goal distribution)
  - BTTS: после обучения стэкинга на BTTS features или Dirichlet calibration
    для BTTS вероятностей
  - MAX_ODD > 2.50: после доказательства calibration quality (error < 0.03)
    для диапазона odds > 2.50
- **Статус**: Принято
